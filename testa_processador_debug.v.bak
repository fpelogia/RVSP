module testa_unid_controle(clk,ALUOp,ALUSrc, n, z, SeltipoSouB, MemToReg, MemWrite, PCSrc,regWrite, Tipo_Branch, rl1,rl2,rd,dado, inst, ula_in2,imed,rl1out, rl2out,imed_p2muxed, ulares, memout, atualPC,novoPC, selSLT_JAL, breg_in);
/*
...
unid_controle uc(.f7(inst[31:25]), .f3(inst[14:12]), .opcode(inst[6:0]), .regWrite(regWrite), .ALUSrc(ALUSrc), .SeltipoSouB(SeltipoSouB), .MemToReg(MemToReg), .MemWrite(MemWrite),.PCSrc(PCSrc), .ALUOp(ALUOp), .Tipo_Branch(Tipo_Branch), .selSLT_JAL(selSLT_JAL));
